# -*- coding: utf-8 -*-
from weather_bot_app.helpers import TextCommandEnum, CacheKey, CacheKeyValue, TsdRegExp

__author__ = 'forward'

import re
from weather_bot_app import logic
...
def initialize_bot_with_routing2(token, chat_id):
    ...
    # блок отвечающий за обратную связь ######################################################
    ...
    # bv.shop_telebot.simple_message_handler(func=lambda message: message.text.lower().startswith(TextCommandEnum.CATALOG.lower()), content_types=['text'], handler=bv.handle_catalog)
    ...
    # bv.shop_telebot.simple_message_handler(func=lambda message: message.text.lower().startswith(TextCommandEnum.SALE.lower()), content_types=['text'], handler=bv.handle_catalog_discount)
    # bv.shop_telebot.simple_callback_query_handler(func=lambda call: call.data.lower().startswith(TextCommandEnum.SALE.lower()), handler=bv.handle_more_catalog_discount_products)
